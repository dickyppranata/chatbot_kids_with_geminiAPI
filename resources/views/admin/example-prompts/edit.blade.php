...
@section('title', 'Edit Example Prompt - Admin AI Buddy')
@section('page_heading', 'Edit Contoh Pertanyaan')
@section('page_subheading', 'Perbarui teks contoh saran pertanyaan dan topik terkait')
...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
            ...
            <div>
                <h3 class="font-outfit font-extrabold text-lg text-slate-900">Edit Example Prompt ID #{{ $examplePrompt->id }}</h3>
                <p class="text-xs text-slate-500 font-medium">Perbarui teks saran pertanyaan atau ubah topiknya.</p>
            </div>
        </div>

        <form action="{{ route('admin.example-prompts.update', $examplePrompt->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                >
                    @foreach($topics as $topic)
                        <option value="{{ $topic->id }}" {{ old('topic_id', $examplePrompt->topic_id) == $topic->id ? 'selected' : '' }}>
            ...
            <div class="space-y-1.5">
                ...
                <input
                    ...
                    id="question_text"
                    required
                    value="{{ old('question_text', $examplePrompt->question_text) }}"
