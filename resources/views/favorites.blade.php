...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/60 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fade-in">
        ...
        <div>
            <a href="/chat" class="px-5 py-3 rounded-full bg-gradient-to-r from-terracotta to-orange-600 hover:from-terracotta-hover hover:to-orange-700 text-white font-outfit font-bold text-xs shadow-md transition-all duration-200 inline-flex items-center gap-2 no-underline">
    ...
    <!-- Favorites Grid List (Server-Side Rendered) -->
    <div id="favoritesListContainer" class="space-y-4">
        ...
        @else
            ...
            @foreach($favorites as $item)
                @php
                    $emoji = $topicEmojis[$item['topic_name']] ?? '📖';
                    $date = \Carbon\Carbon::parse($item['created_at'])->locale('id')->isoFormat('D MMMM Y');
                @endphp
